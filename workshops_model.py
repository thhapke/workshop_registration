...
def get_workshops(user_id,incl_ended = False) :
    ...
    if incl_ended :
        sql_command = "SELECT ID,TITLE,WORKSHOP_START,REGISTRATION_START,REGISTRATION_END,MAX_USER, TENANT, URL "\
                      "FROM DIREGISTER.WORKSHOPS WHERE MODERATOR=\'{}\';".format(user_id)
    else :
        sql_command = "SELECT ID,TITLE,WORKSHOP_START,REGISTRATION_START,REGISTRATION_END,MAX_USER, TENANT, URL "\
                      "FROM DIREGISTER.WORKSHOPS WHERE REGISTRATION_END > \'{}\' AND MODERATOR=\'{}\';".format(nowtime,user_id)
    ...
    if len(rows) > 0 :
        ...
        wss = {r[0]:{'ID':r[0],'Title':r[1],'Moderator':user_id,'Workshop Start':r[2],'Regist. Start':r[3],'Regist. End':r[4],'Max. User':r[5],\
...
def get_workshop(ws_id) :
    ...
    sql_command = "SELECT ID,TITLE,WORKSHOP_START,REGISTRATION_START,REGISTRATION_END,MAX_USER, TENANT, URL "\
                  "FROM DIREGISTER.WORKSHOPS WHERE ID = \'{}\';".format(ws_id)
    cursor = conn.cursor()
    cursor.execute(sql_command)
    row = cursor.fetchone()
    cursor.close()
    conn.close()
    event = {'ID':row[0],'Title':row[1],'Moderator':"",'Workshop Start':row[2],'Regist. Start':row[3],'Regist. End':row[4],'Max. User':row[5],\
